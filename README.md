...
- 审查前先统计剧集正文有效字数。
- 有效字数不足 4000 字时，直接判定为“字数不达标 / 阻断”，不进入评分、分场审核或修改建议。
...
`/review` 命令先执行字数门槛检查；单集有效字数达到 4000 字后，才进入五维度评分。每个维度 1-10 分，总分 50 分。

### 前置门槛

| 项目 | 标准 | 不达标处理 |
|------|------|------------|
| 有效字数 | 单集正文 ≥ 4000 字 | 停止后续审查，先补足字数后重新 `/review` |
